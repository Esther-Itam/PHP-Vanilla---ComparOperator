...
$allUsername = $pdo->query('SELECT DISTINCT * 
                                FROM tour_operators
                                LEFT JOIN users
                                ON users.username = tour_operators.name 
                                WHERE users.username IS NULL');

$selectUsernames = $allUsername->fetchAll();
...
if (isset($valider)) {
   ...
   elseif ($password != $repassword) $erreur = "Mots de passe non identiques!";
      ...
      if (count($tab) > 0)
         ...
         $ins = $pdo->prepare("INSERT INTO users(username, password, id_tour_operator) VALUES(?,?,?)");
         if ($ins->execute(array($username, md5($password), $id_tour_operator)))
               ...
               <form name="fo" method="POST" action="">
                  ...
                  <input type="hidden" name="id_tour_operator" value="1">
                     ...
                     <select name="username">
                        ...
                        <?php foreach ($selectUsernames as $selectUsername) { ?>
                           <option name="username" value="<?= $selectUsername["name"] ?>"><?= $selectUsername["name"] ?></option>
                        <?php } ?>
                     </select>
